...
java -jar target/movie-service-1.0.0.jar --spring.profiles.active=develop
java -jar target/movie-service-1.0.0.jar --spring.profiles.active=test
java -jar target/movie-service-1.0.0.jar --spring.profiles.active=staging
java -jar target/movie-service-1.0.0.jar --spring.profiles.active=prod
```
...
export DB_URL="jdbc:postgresql://localhost:5432/back-ms-movies_db"
...
# View logs
docker-compose logs -f back-ms-movies-app
...
- **pgAdmin**: http://localhost:5050 (admin@back-ms-movies.local/admin123)

#### Production Docker Build
```bash
# Build optimized image
docker build -t back-ms-movies:latest .
...
docker run -d \
  ...
  -e DB_URL="jdbc:postgresql://your-db:5432/back-ms-movies_db" \
  -e DB_USERNAME="your_user" \
  -e DB_PASSWORD="your_password" \
  back-ms-movies:latest
```
...
- **Port:** 8080
- **Database:** PostgreSQL (localhost:5432/back-ms-movies_db)
