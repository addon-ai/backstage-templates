...
🚀 Reactive microservice for users management with Spring WebFlux
...
│   └── UserServiceWebFluxApplication.java
└── test/                    # Test Classes
```
...
mvn spring-boot:run
# or
java -jar target/user-service-webflux-1.0.0.jar
```
...
- **main**: main environment with environment variables
...
java -jar target/user-service-webflux-1.0.0.jar --spring.profiles.active=develop
java -jar target/user-service-webflux-1.0.0.jar --spring.profiles.active=test
java -jar target/user-service-webflux-1.0.0.jar --spring.profiles.active=staging
...
export DB_URL="jdbc:postgresql://localhost:5432/back-ms-users-webflux_db"
...
# View logs
docker-compose logs -f back-ms-users-webflux-app
...
- **pgAdmin**: http://localhost:5050 (admin@back-ms-users-webflux.local/admin123)

#### Production Docker Build
```bash
# Build optimized image
docker build -t back-ms-users-webflux:latest .
...
docker run -d \
  ...
  -e DB_URL="jdbc:postgresql://your-db:5432/back-ms-users-webflux_db" \
  -e DB_USERNAME="your_user" \
  -e DB_PASSWORD="your_password" \
  back-ms-users-webflux:latest
```
...
- **Port:** 8080
- **Database:** PostgreSQL (localhost:5432/back-ms-users-webflux_db)
...
#### main Profile (`application-main.properties`)
- **Database:** Uses `${DB_URL}`, `${DB_USERNAME}`, `${DB_PASSWORD}`
- **JPA:** DDL validate mode, SQL logging disabled
- **Swagger:** Disabled by default
- **Logging:** INFO level
