...
java -jar target/user-service-1.0.0.jar --spring.profiles.active=develop
java -jar target/user-service-1.0.0.jar --spring.profiles.active=test
java -jar target/user-service-1.0.0.jar --spring.profiles.active=staging
...
export DB_URL="jdbc:postgresql://localhost:5432/back-ms-users_db"
...
# View logs
docker-compose logs -f back-ms-users-app
...
- **pgAdmin**: http://localhost:5050 (admin@back-ms-users.local/admin123)

#### Production Docker Build
```bash
# Build optimized image
docker build -t back-ms-users:latest .
...
docker run -d \
  ...
  -e DB_URL="jdbc:postgresql://your-db:5432/back-ms-users_db" \
  -e DB_USERNAME="your_user" \
  -e DB_PASSWORD="your_password" \
  back-ms-users:latest
```
...
- **Port:** 8080
- **Database:** PostgreSQL (localhost:5432/back-ms-users_db)
